...
| `.github/workflows/tag-release.yml` | `push` of a `v*` or `*.*.*` version tag, or a published GitHub Release | Creates a draft GitHub Release, compiles standalone Bun binaries for macOS, Linux, and Windows on x64 and arm64 runners, uploads the six assets, publishes SHA-256 checksums, and then makes the release public. |
...
the version, select the release target, and publish it. Publishing the release starts the build
workflow even when the tag already exists.
